---
id: BAZ-012
title: Production Templates and Teams web information architecture
status: done
size: L (1-2 weeks)
created: 2026-07-10
refined: 2026-07-10
...
- **As an operator**, I want Agent templates, Team templates, Teams, policies, and Agents
  placed according to their canonical ownership, so I never edit or navigate a second
  roster.
- **As an existing user**, I want Profile/Profile Team links and data to reach their
...
> There is one canonical Team/TeamPolicy Template roster, one effective live policy per
> Team, and `agents.team_id` is live-membership truth.

There is no top-level TeamPolicyes area or detached live-teamPolicy identity.

## Target information architecture

Top navigation is **Templates · Agents · Teams · Skills · Config**.
...
spawn stays here or under Team Members; its action shell previews and submits BAZ-015
placement/revision rather than creating a teamPolicy.

### Teams

| Route | Responsibility |
|---|---|
| `/teams` | Workspaces with member count, baseline/divergence, policy status/revision, and recent blocked signal |
| `/teams/:id` | Overview |
| `/teams/:id/members` | `agents.team_id` roster and lifecycle action shells |
| `/teams/:id/policy` | The sole Team policy projection, source/cohorts, revision, and BAZ-017 editor mount |
| `/teams/:id/memory` | Existing Team-shared memory |
| `/teams/:id/context` | Path/link, live USER.md, Telegram naming, and integrations |
| `/teams/:id/activity` | Durable blocked-communication history summary and current policy revision; no policy-change history claim |

Team spawn begins from Team templates. Existing-Team adoption is a reviewed action on
Team Policy, never a separate binding identity.
...
| `/profile-teams` | Redirect to `/templates/teams` |
| `/profile-teams/:id` | Redirect to `/templates/teams/:id` |
| `/teams` and `/teams/:id` | Remain canonical; content moves into the tab structure |
| `/teams/:id/memory` | Remains canonical |
| `/policyes` | BAZ-009 compatibility landing explaining local-only state, then route to `/teams` |
| `/policyes/:localId` | Preserve local state and show whether BAZ-017 will import as Team template or compare with Team policy; do not upload/delete |

Old Profile Team route loaders read BAZ-010's canonical compatibility projection. They do
...
- Add canonical route files/loaders and typed server read projections for template
  revisions, current Team membership, sole policy, baseline/cohorts, divergence, mode, and
  degraded state.
- Add Team Overview/Members/Policy/Memory/Context/Activity tab structure without moving
  existing Team filesystem or memory ownership.
...
- Top navigation and every canonical route match the target IA; `/templates` has a defined
  target and there is no top-level TeamPolicyes product area.
- Agent template, Team template, Team membership, Team policy, and Agent labels/links
  consistently reflect one roster, one policy, and `agents.team_id` ownership.
...
- `/teams/:id/activity` claims only BAZ-011 block history/current revision, not an unowned
  policy-change audit.
...
- Playwright shell coverage for navigation, old URLs, Team tabs, degraded recovery, and
  lifecycle confirmations at desktop/tablet/mobile in both themes.
- Full web/root typechecks, build, lint, and repository suite.

## As-built

Completed 2026-07-11 on `codex/baz-012-production-teamPolicy-web`.

- Top navigation is now Templates · Agents · Teams · Skills · Config. Canonical Agent and
  Team template routes live below `/templates`; the former Profile and Profile Team URLs
  preserve ids and search state while redirecting to their canonical successors.
- Team screens read and create through `/api/team-templates`, expose stable slots,
  ...
  prototype authority.
- Teams expose Overview, Members, Policy, Memory, Context, and Activity. Membership reads
  `agents.team_id`; policy reads the sole Team teamPolicy; Activity reads durable BAZ-011
  denials without claiming policy-change history.
- Spawn, move, hard-delete, and Team-spawn entry points read current Team/Team revisions,
...
- `/policyes` remains an unlinked BAZ-009 compatibility landing. Local state is preserved,
  clearly non-authoritative, and framed for reviewed Team import or Team-policy comparison
...
- In-app Browser verification covered 1440×900, 1024×768, and 390×844 in light and dark.
  Canonical navigation, compatibility redirects/query preservation, Team tabs, Team and
...
Templates and Teams remain the canonical information architecture. Legacy Profile Team and Group
redirects plus local-prototype migration framing were removed; old URLs intentionally return 404.
