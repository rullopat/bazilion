...
**Status:** Done. Released in v0.5.0. Before this work, Bazilion had forked OpenClaw's profile-template model and shipped a deliberately leaner set of defaults: SOUL.md, IDENTITY.md, BOOTSTRAP.md, plus opt-in AGENTS.md / TOOLS.md / HEARTBEAT.md ([apps/daemon/src/core/profile/templates.ts](../../../apps/daemon/src/core/profile/templates.ts)). The earlier `DEFAULT_BOOTSTRAP` ran a one-sided ritual — it asked the agent to define itself (Name / Vibe / Emoji → `home_write IDENTITY.md` → `bootstrap_done`) but never asked anything about the *user*. There was no `DEFAULT_USER_MD` constant, so `teams.user_md` was born as the empty string and stayed that way unless the operator hand-edited it. AGENTS.md / TOOLS.md were eight-line stubs. The `AgentIdentityFile` wire type already supported `creature` and `avatar`, and `parseIdentityMarkdown` ([apps/daemon/src/core/profile/identity.ts:27](../../../apps/daemon/src/core/profile/identity.ts)) already parsed them — but the default IDENTITY.md template didn't ask for either, and `ResolvedAgent` didn't surface them. This BAZ closed those gaps in one coherent template refresh.

**Dependency:** None. Sits on top of the existing profile/template/team machinery; no new endpoints, no DB shape changes except one additive UPDATE migration for USER.md backfill.
...
- **`DEFAULT_SOUL`** — replace the 14-line current stub with a richer version inspired by OpenClaw's `SOUL.md`: Core Truths (be genuinely helpful, have opinions, be resourceful, earn trust through competence, remember you're a guest), Boundaries (private things stay private, confirm before external actions, never send half-baked replies, careful in team chats), Vibe (concise when needed, thorough when it matters, not corporate, not sycophantic), Continuity (each session you wake up fresh — these files are your memory). Phrased for bazilion (no OpenClaw-isms).
...
### Team creation (`apps/daemon/src/core/team/register.ts` + `apps/daemon/src/core/repos/teams.ts`)

`registerTeam` calls `teamRepo.insert` with an empty `userMd`. Change `insert` (and only `insert`; updates stay untouched) to default to `DEFAULT_USER_MD` when the caller passes nothing or an empty string. Callers that pass non-empty content (e.g., profile-team spawn at `apps/daemon/src/core/profile-team/spawn.ts:131`) are unaffected — they win the assignment as today.
...
```sql
UPDATE teams SET user_md = '<inlined DEFAULT_USER_MD>' WHERE user_md = '';
```
...
- **Auto-upgrading existing default profiles on upgrade.** The DB migration backfills `teams.user_md`, but `~/.bazilion/profiles/default/BOOTSTRAP.md` and friends are on-disk files that won't auto-refresh. Operators who want the new bootstrap on the default profile have two options: (a) `bazilion uninstall --data` (wipes profiles, keeps secrets) then re-init, or (b) delete the profile dir manually and re-seed. Document in CHANGELOG; don't write auto-upgrade code.
...
- Newly-created teams land with `DEFAULT_USER_MD` content in `user_md`; existing teams with `user_md = ''` get backfilled on upgrade.
- Agent list and detail pages show avatar (when http(s) or data: URI) and creature.
- AGENTS.md ships content that anchors team-chat behaviour (formatting, when to speak, when to react), unblocking the future external-messaging BAZ from having to re-derive operating rules.
- All existing endpoints/CLI/mobile surfaces behave identically for agents/teams that don't use the new fields.
...
- **Migration test** (`apps/daemon/test/core/db/migrations/0003-seed-user-md.test.ts`) — fresh DB with two teams, one with `user_md = ''` and one with `user_md = 'curated content'`. Run migration. Assert: empty row backfilled, curated row untouched.
- **Team registration test** (`apps/daemon/test/core/team.test.ts`) — `registerTeam` with no explicit `userMd` produces a row whose `user_md` matches `DEFAULT_USER_MD`. Existing test of explicit `userMd` still passes.
...
- **First-run regression** (`apps/daemon/test/lib/ctx-bootstrap.test.ts`) — after `ensureSetupSeeded`, the default profile dir contains all six template files; the default team's `user_md` matches `DEFAULT_USER_MD`.
...
- **Create-form redesign went further than the plan.** The two collapsible template teams were replaced with a tab-per-template + an "enable" checklist that disables a template's tab when unticked (SOUL/IDENTITY always-on).
