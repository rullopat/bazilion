...
- **As an operator investigating denial**, I want durable block detail and a side-effect-free
  simulator in the Team where policy is owned.
...
- Keep Team-template stable slot ids and Team Agent ids distinct in labels, selection,
...
- Add reviewed new/empty Team initialize, non-empty/retained-baseline append, and full
  existing-Team rebaseline workflows with exact resulting-edge preview.
...
- Replace simulated local blocks with paginated BAZ-011 events. Show both Team policy
  components for cross-Team denial. Keep diagnostic evaluation clearly side-effect free.
- Add reviewed BAZ-009 export/import: server creates new template/slot ids; a local live
  draft compares against its Team with expected revision; nothing uploads/deletes
  silently; simulated blocks never import.
- Preserve Agent chat drill-in and restore Team policy route, Flow/Matrix mode, viewport,
...
- Team Policy shows one baseline pointer, cohort provenance, live-only Agents, divergence,
  revision, and source tombstone state without implying auto-propagation.
- Cross-Team simulation shows both current Team revisions; editing one Team cannot grant
...
- Block detail shows source/target/channel/origin/reason/time plus every policy reference,
  revision, and cross-Team component. Simulator creates no block.
...
- Playwright end-to-end coverage for create/edit/reload, Flow/Matrix sync, Team
  initialize/append, Team rebaseline, multi-browser conflict, simulator/activity, local
...
- Replaced the BAZ-009 local prototype projection with canonical server-backed Flow and
  Matrix editors for Team templates and Team live policy. Both projections share one edge
...
- Added exact daemon previews for Team initialize/append, template adoption/rebaseline,
  direct placement, and Agent moves. Team policy exposes baseline/current-source/live
...
- Added reviewed local export/import. Imported Team slots always receive server UUIDs,
  invalid documents fail atomically, local Team drafts compare before expected-revision
  application, and local simulated blocks never enter durable production activity.
- Expanded Team activity into paginated durable denial detail with source, target,
  channel, origin, reason, time, attempt, policy references, revisions, both cross-Team
  components, and matched/required edges. Agent drill-in restores Team route, projection,
...
The production Team Template and Team Policy editors, activity, conflicts, and recovery remain.
The BAZ-009 local migration path was removed together with the browser-local prototype.
