...
**Status:** Done. Released in v0.2.0. At refinement time, a `Profile` ([packages/api-types/src/entities.ts:26](../../../packages/api-types/src/entities.ts)) was a single-agent template — SOUL.md + IDENTITY.md + `defaultModel` + `skillsMode` — and `spawnAgent` ([apps/daemon/src/core/agent/spawn.ts](../../../apps/daemon/src/core/agent/spawn.ts)) was invoked once per agent via `POST /api/agents`. There was no notion of "team", "squad", "preset", or "bundle" in the daemon. Standing up a new project meant manually spawning each agent, picking a profile, naming it, and assigning a team — for a four-to-six-agent team that was a fifteen-minute click-fest the operator repeated verbatim every project. This BAZ added a `ProfileGroup` entity that captured the recipe once and replayed it with one call.
