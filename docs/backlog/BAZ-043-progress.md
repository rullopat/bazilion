...
| 4 | Operator findings + conclusion | API, CLI and web for opening a packet, adding findings with severity and line context, recording a conclusion, and resolving a finding only by explicit decision or a linked revision | **API + CLI done**, web pending |
...
- **The web section.** API and CLI are done; the web view is not, so criterion 6's "API/CLI and web
  share review status and exports" is only half met.
