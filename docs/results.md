...
archive matches the current release's full migration chain, so archives from before the
forward-migration contract require the documented reset workflow rather than an import.
