...
export function ConversationLibrary({ agentId, turnBusy, onCreated, renderHistory, initialConversationId }: {
  ...
  return <section aria-live="off" className="my-3 rounded-lg border border-fawn bg-ivory p-4" aria-label="Conversation library">
    ...
    <p className="text-sm">Starting a new conversation keeps previous history and saved files. Viewing history does not change the active conversation.</p>
    ...
    {!library ? <p role="status">Loading conversations…</p> : <>
      ...
      <ul className="m-0 grid list-none gap-2 p-0">
        {library.conversations.map(item => <li key={item.id} className="flex min-w-0 flex-wrap items-center justify-between gap-2 rounded border border-fawn p-2">
          <span className="min-w-0 break-words">{item.title}{item.id === library.selection.conversationId && <strong className="ml-2 text-sm">Active</strong>}</span>
          <Button variant="ghost" className="max-w-full whitespace-normal break-words text-left" disabled={busy} aria-pressed={selected?.id === item.id} onClick={() => { setSelected(item); setTitle(item.title) }}>View {item.title}</Button>
        </li>)}
      </ul>
    ...
    {selected && <section className="mt-4 border-t border-fawn pt-4" aria-label="Retained conversation">
      <div className="flex flex-wrap items-end gap-2">
        <label className="min-w-0 flex-1">Conversation title<input value={title} maxLength={200} onChange={event => setTitle(event.target.value)} /></label>
        <Button variant="ghost" disabled={busy || !title.trim()} onClick={() => void rename()}>Rename</Button>
        <Button variant="ghost" disabled={busy} onClick={() => setSelected(null)}>Close history</Button>
      </div>
      <p className="text-sm">Viewing retained history. New messages still go to the active conversation.</p>
