import type { TeamTemplateDetail, TeamTemplateWithCount, TeamPolicyEdgeInput, ResolvedTeamPolicy } from '@bazilion/api-types'
...
export function TeamPolicyOperations({ teamId, detail, templates }: { teamId: string; detail: ResolvedTeamPolicy; templates: TeamTemplateWithCount[] }) {
  const [diff, setDiff] = useState<TeamPolicyDiff | null>(null)
  const [error, setError] = useState<string | null>(null)
  const [busy, setBusy] = useState(false)
  const [templateId, setTemplateId] = useState(`${teamId}-snapshot`)
  const [templateName, setTemplateName] = useState(`${teamId} snapshot`)
  ...
  const reloadDiff = async () => {
    const response = await fetch(`/api/teams/${encodeURIComponent(teamId)}/policy/diff`)
  ...
  useEffect(() => { void reloadDiff().catch((cause) => setError(cause instanceof Error ? cause.message : String(cause))) }, [teamId])

  const updateSource = async () => {
    if (!diff?.currentSource || diff.sourceDiverged) return
    setBusy(true); setError(null)
    try {
      const response = await fetch(`/api/teams/${encodeURIComponent(teamId)}/policy/update-source`, { method: 'POST', headers: {'content-type':'application/json'}, body: JSON.stringify({ teamExpectedRevision: detail.teamPolicy.revision, templateExpectedRevision: diff.currentSource.template.currentRevision, includeAgentIds: liveOnly.map((agent) => agent.id) }) })
  ...
  const saveAsTemplate = async () => {
    setBusy(true); setError(null)
    try {
      const response = await fetch(`/api/teams/${encodeURIComponent(teamId)}/policy/save-as-template`, { method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({expectedRevision:detail.teamPolicy.revision,id:templateId,name:templateName}) })
      if (!response.ok) throw new Error((await response.json().catch(()=>null))?.error ?? `Save failed (${response.status})`)
      const saved = (await response.json()) as TeamTemplateDetail
  ...
  return <section className="card space-y-4" aria-labelledby="source-workflows"><div className="flex flex-wrap items-start gap-3"><GitCompareArrows className="mt-1 h-5 w-5 text-sapphire"/><div className="mr-auto"><h2 id="source-workflows" className="m-0 text-lg">Baseline and source</h2><p className="muted mt-1 text-sm">Lineage is retained for comparison; source edits never auto-propagate.</p></div><Button variant="ghost" onClick={()=>void reloadDiff()}><RefreshCw className="h-4 w-4"/>Refresh comparison</Button></div>
    ...
    {diff?.sourceDiverged && <div role="alert" className="rounded-md border border-warning/25 bg-warning/10 p-3 text-sm text-warning">The source has changed since this Team retained its baseline. Both versions remain intact. Update-source is disabled; compare and explicitly rebaseline instead.</div>}
    ...
    <AdoptionWorkflow teamId={teamId} detail={detail} templates={templates} />
  </section>
}

function Metric({label,value,warning=false}:{label:string;value:string;warning?:boolean}) { return <div className={`rounded-md border p-3 ${warning?'border-warning/25 bg-warning/10 dark:bg-amber-950/30':'border-frost bg-ivory'}`}><dt className="text-xs font-semibold uppercase text-mocha-light">{label}</dt><dd className="mt-1 break-words text-sm font-semibold">{value}</dd></div> }

function AdoptionWorkflow({ teamId, detail, templates }: { teamId: string; detail: ResolvedTeamPolicy; templates: TeamTemplateWithCount[] }) {
  const [templateId, setTemplateId] = useState(templates.find((item) => !item.deletedAt)?.id ?? '')
  const [template, setTemplate] = useState<TeamTemplateDetail | null>(null)
  const [mapping, setMapping] = useState<Record<string, string>>({})
  const [placements, setPlacements] = useState<Record<string, 'isolated'|'profile_defaults'>>({})
  const [preview, setPreview] = useState<TeamPolicyEdgeInput[] | null>(null)
  const [error, setError] = useState<string | null>(null)
  const loadTemplate = async () => {
    setError(null); setPreview(null)
    const response = await fetch(`/api/team-templates/${encodeURIComponent(templateId)}`)
    if (!response.ok) { setError(`Could not load Team (${response.status})`); return }
    const loaded = (await response.json()) as TeamTemplateDetail
    setTemplate(loaded)
  ...
  const review = async () => {
    ...
    const response = await fetch(`/api/teams/${encodeURIComponent(teamId)}/policy/adopt-template/preview`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(request())})
  ...
  const adopt = async () => {
    if (!preview) return
    const response=await fetch(`/api/teams/${encodeURIComponent(teamId)}/policy/adopt-template`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({...request(),previewEdges:preview})})
