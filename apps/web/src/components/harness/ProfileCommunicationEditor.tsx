...
const PEER_OPTIONS: Array<{
  ...
  {
    value: 'inherit_harness',
    label: 'Inherit harness',
    description: 'Keep peer edges created by the selected harness preset.',
  },
  {
    value: 'allow_all',
...
export function ProfileCommunicationEditor({
  ...
  const gates: Array<{
    key: keyof Pick<
      ProfileCommunicationDefaults,
      'userInput' | 'userOutput' | 'outsideGroupInput' | 'outsideGroupOutput'
    >
    label: string
    detail: string
    ...
    {
      key: 'outsideGroupInput',
      label: 'Other-group input',
      detail: 'Agents in other local groups may send to this agent.',
      icon: UserRound,
    },
    {
      key: 'outsideGroupOutput',
      label: 'Other-group output',
      detail: 'This agent may send to agents in other local groups.',
      icon: Network,
    },
  ]
