...
type Source =
  | { kind: 'template'; detail: HarnessTemplateDetail }
  | { kind: 'live'; groupId: string; detail: ResolvedGroupHarness }

export function ProductionHarnessEditor({ source, profiles, initialUi }: { source: Source; profiles: Profile[]; initialUi?: { view?: 'flow'|'matrix'; selectedId?: string|null; viewport?: {x:number;y:number;zoom:number} } }) {
  const initial = useMemo(
    () => source.kind === 'template' ? templateDocument(source.detail) : liveDocument(source.groupId, source.detail),
    [source],
  )
  const [server, setServer] = useState(initial)
  const [draft, setDraft] = useState(initial)
  const [revision, setRevision] = useState(
    source.kind === 'template' ? source.detail.template.currentRevision : source.detail.harness.revision,
  )
  ...
  const [conflict, setConflict] = useState<HarnessDocument | null>(null)
  ...
  const mutateEdge = (from: HarnessEndpoint, to: HarnessEndpoint, allowed: boolean) => {
    if (!isValidHarnessConnection(from, to)) return
    setDraft((current) => ({
      ...current,
      policy: allowed ? addHarnessEdge(current.policy, from, to) : removeHarnessEdge(current.policy, from, to),
    }))
  }
  ...
  const reloadCurrent = async (): Promise<{ document: HarnessDocument; revision: number }> => {
    const url = source.kind === 'template'
      ...
      : `/api/groups/${encodeURIComponent(source.groupId)}/harness`
    const response = await fetch(url)
    if (!response.ok) throw new Error(await response.text())
    if (source.kind === 'template') {
      const detail = (await response.json()) as HarnessTemplateDetail
      return { document: templateDocument(detail), revision: detail.template.currentRevision }
    }
    const detail = (await response.json()) as ResolvedGroupHarness
    return { document: liveDocument(source.groupId, detail), revision: detail.harness.revision }
  }
  ...
  const save = async () => {
    ...
    try {
      const response = source.kind === 'template'
        ? await fetch(`/api/harness-templates/${encodeURIComponent(source.detail.template.id)}/definition`, {
            method: 'PUT', headers: { 'content-type': 'application/json' },
            body: JSON.stringify({ expectedRevision: revision, ...templateDefinition(draft, profiles) }),
          })
        : await fetch(`/api/groups/${encodeURIComponent(source.groupId)}/harness/policy`, {
            method: 'PUT', headers: { 'content-type': 'application/json' },
    ...
    <section className="overflow-hidden rounded-lg border border-frost bg-snow" aria-label="Production policy editor">
      <header className="flex flex-wrap items-center gap-2 border-b border-frost bg-cream p-3">
        <div className="mr-auto min-w-0">
          ...
          <p className="muted m-0 text-xs">{source.kind === 'template' ? 'Stable Team slot policy' : 'One effective live Group policy'}</p>
        </div>
...
function validSelection(document: HarnessDocument, selectedId: string | null | undefined): string | null {
  if (!selectedId) return null
  if (selectedId === 'user' || selectedId === 'outside_group') return selectedId
...
function ConflictBanner({ draft, server, onReload, onReapply }: { draft: HarnessDocument; server: HarnessDocument; onReload: () => void; onReapply: () => void }) {
...
function Inspector({ document, selected, selectedMember, profiles, source, onDraft, onToggle, onPosture }: { document: HarnessDocument; selected: HarnessEndpoint | null; selectedMember?: HarnessDocument['members'][number]; profiles: Profile[]; source: Source; onDraft: React.Dispatch<React.SetStateAction<HarnessDocument>>; onToggle: (a: HarnessEndpoint,b: HarnessEndpoint,c:boolean) => void; onPosture: (a: HarnessEndpoint,b: HarnessEndpoint,p:'allow'|'approval_required')=>void }) {
  ...
  const actors = [{ key: 'user', label: 'User' }, ...document.members.map((m) => ({key: endpointKey(endpointForMember(document,m)), label: m.name})), {key:'outside_group',label:'Other groups'}]
  ...
  const simulate = async () => {
    ...
    if (source.kind !== 'live') { setResult({decision: hasHarnessEdge(document.policy,from,to)?'allow':'deny', channel:'user', reasonCode:'draft_preview', reason:'Draft-only preview; no audit event was created.', policyRefs:[], componentOutcomes:[], matchedEdgeIds:[], requiredEdgeIds:[]}); return }
    const endpoint = (value: HarnessEndpoint) => value.kind === 'agent' ? {kind:'agent' as const,id:value.agentId} : {kind:value.kind,groupId:source.groupId}
