...
interface BoundaryNodeData extends Record<string, unknown> {
  label: string
  detail: string
  kind: 'user' | 'outside_group'
}

type HarnessFlowNode = FlowNode<AgentNodeData, 'agent'> | FlowNode<BoundaryNodeData, 'boundary'>
...
interface HarnessFlowProps {
  ...
  onConnect: (source: HarnessEndpoint, target: HarnessEndpoint) => void
  onRemoveEdge: (source: HarnessEndpoint, target: HarnessEndpoint) => void
  onMoveMember: (slotId: string, position: HarnessPosition) => void
  onViewportChange: (viewport: Viewport) => void
  onOpenMember: (member: HarnessMember) => void
}
...
export function HarnessFlow({
  ...
  const computedNodes = useMemo<HarnessFlowNode[]>(() => {
    const outsideX = Math.max(930, ...harness.members.map((member) => member.position.x + 280))
    const boundaryNodes: HarnessFlowNode[] = [
      {
      ...
      },
      {
        ...
        data: { label: 'Other groups', detail: 'Local Bazilion agents', kind: 'outside_group' },
        ariaLabel: 'Other local groups communication boundary',
        selected: selectedId === endpointKey(OUTSIDE_GROUP_ENDPOINT),
        draggable: false,
        deletable: false,
      },
    ]
    const memberNodes = harness.members.map<HarnessFlowNode>((member) => {
      ...
      return {
        ...
        data: {
          ...
          live: harness.kind === 'live',
        },
        ariaLabel: `${member.name}, ${isolated ? 'isolated' : 'configured'}${incompleteSlotIds.has(member.slotId) ? ', incomplete' : ''} ${harness.kind === 'live' ? 'agent' : 'member slot'}`,
        selected: selectedId === endpointKey(endpoint),
        deletable: false,
      }
    })
    return [...boundaryNodes, ...memberNodes]
  }, [harness, incompleteSlotIds, selectedId])
  const [nodes, setNodes, onNodesChange] = useNodesState<HarnessFlowNode>(computedNodes)
  ...
  const edges = useMemo<FlowEdge[]>(() => {
    const simulationKey = simulatedPath
      ...
      : null
    const policyEdges = harness.policy.edges.map((edge) => {
  ...
  }, [harness.policy.edges, selectedId, simulatedPath])
