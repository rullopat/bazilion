            ...
            {actors.map((target) => (
              ...
              >
                <button
                  ...
                  title={harnessEndpointLabel(harness, target)}
                >
                  {harnessEndpointLabel(harness, target)}
                </button>
              </th>
            ))}
          </tr>
            ...
            <tr key={endpointKey(source)}>
              <th className="sticky left-0 z-10 max-w-40 bg-snow normal-case tracking-normal">
                <button
                  ...
                  title={harnessEndpointLabel(harness, source)}
                >
                  {harnessEndpointLabel(harness, source)}
                </button>
              </th>
              {actors.map((target) => {
                const valid = isValidHarnessConnection(source, target)
                const allowed = valid && hasHarnessEdge(harness.policy, source, target)
                const label = `${harnessEndpointLabel(harness, source)} to ${harnessEndpointLabel(harness, target)}`
                return (
