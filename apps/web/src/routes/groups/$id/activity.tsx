...
export const Route = createFileRoute('/groups/$id/activity')({
  loader: ({ params }) => fetchActivity({ data: params }),
  component: ActivityPage,
  errorComponent: ({ error, reset }) => <RecoveryState title="Policy activity unavailable" error={error} reset={reset} fallbackHref="/groups" />,
})
...
function ActivityPage() {
  ...
  const loadMore = async () => {
    ...
    try {
      const response = await fetch(`/api/groups/${encodeURIComponent(id)}/harness/blocks?limit=25&cursor=${encodeURIComponent(page.nextCursor)}`)
  ...
  return <div className="mx-auto max-w-5xl space-y-5 px-4 py-6 sm:px-6"><div><h1>blocked communication</h1><p className="muted">Durable denial evidence. Diagnostic simulations never create these records.</p></div><GroupTabs groupId={id} />
    <div className="space-y-3">{page.blocks.map((block) => <BlockCard key={block.id} block={block} />)}
      {page.blocks.length === 0 && <p className="card muted">No blocked communication has been recorded for this group.</p>}
    </div>
...
function BlockCard({ block }: { block: BlockEvent }) {
  return <details className="card group"><summary className="flex cursor-pointer list-none flex-wrap items-center gap-2"><strong className="mr-auto break-all">{block.reason_code}</strong><time className="text-xs text-mocha-light">{new Date(block.created_at).toLocaleString()}</time><ChevronDown className="h-4 w-4 transition-transform group-open:rotate-180" /></summary>
    ...
    <h3 className="mt-5 text-sm">Policy revisions</h3><ul className="mt-2 text-sm">{block.policyRefs.map((ref) => <li key={`${ref.groupId}:${ref.revision}`}><code>{ref.groupId}</code> revision {ref.revision}</li>)}</ul>
    {block.componentOutcomes.length > 1 && <><h3 className="mt-5 text-sm">Cross-Group consent components</h3><div className="mt-2 overflow-x-auto"><table><thead><tr><th>Group</th><th>Required edge</th><th>Matched</th></tr></thead><tbody>{block.componentOutcomes.map((item) => <tr key={`${item.groupId}:${item.edge}`}><td><code>{item.groupId}</code></td><td className="break-all"><code>{item.edge}</code></td><td>{item.matched ? 'yes' : 'no'}</td></tr>)}</tbody></table></div></>}
