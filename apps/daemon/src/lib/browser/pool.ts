...
/**
 * The browser's *own* default user-agent — generated by the real Chromium for
 * this exact machine (OS, arch, version) — with the headless tell removed. This
 * is strictly more faithful than reconstructing the platform string ourselves:
 * no hardcoded OS table, no version drift, no arch mismatch. We only swap the
 * `HeadlessChrome` token (present only in headless mode) for `Chrome`. Falls
 * back to a constructed UA if the probe fails.
 */
async function resolveUserAgent(browser: Browser): Promise<string> {
  try {
    ...
    // fall through to the constructed UA
  }
  return constructedUserAgent(browser.version())
}

/** Fallback UA, consistent with the running engine version + host OS. */
function constructedUserAgent(version: string): string {
...
async function createSession(agentId: string, config: BrowserConfig): Promise<BrowserSession> {
  ...
  const context = await browser.newContext({
    // Use the real browser's own UA (true OS/arch/version for this machine)
    // with the headless tell stripped. A mismatched UA contradicts the engine
    // fingerprints a real browser exposes and *raises* bot-detection signal.
