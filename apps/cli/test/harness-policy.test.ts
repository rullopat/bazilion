...
beforeAll(async () => {
  ...
  scratch = mkdtempSync(join(tmpdir(), 'bazilion-harness-cli-'))
})
afterAll(async () => {
  await server.stop()
...
test('Team import requires review/apply, allocates server ids, and exports portable JSON', async () => {
  ...
  let result = await server.cli(['team', 'import', path, '--dry-run'])
  expect(result.exitCode).toBe(0)
  expect(result.stdout).toContain('valid: no changes applied')

  result = await server.cli(['team', 'import', path])
  expect(result.exitCode).toBe(2)
  expect(result.stderr).toContain('refusing mutation')

  result = await server.cli(['team', 'import', path, '--apply'])
  expect(result.exitCode).toBe(0)
  expect(result.stdout).toContain('created Team ops at revision 1')

  result = await server.cli(['team', 'export', 'ops'])
  expect(result.exitCode).toBe(0)
  ...
  result = await server.cli(['team', 'show', 'ops', '--json'])
...
test('existing Team replacement requires an expected revision and stale writes fail', async () => {
  ...
  expect((await server.cli(['team', 'import', path, '--apply'])).exitCode).toBe(0)

  let result = await server.cli(['team', 'import', path, '--apply'])
  expect(result.exitCode).toBe(2)
  expect(result.stderr).toContain('--expected-revision is required')

  result = await server.cli(['team', 'import', path, '--apply', '--expected-revision', '99'])
  expect(result.exitCode).toBe(3)
  expect(result.stderr).toContain('template_revision_conflict')

  result = await server.cli(['team', 'import', path, '--apply', '--force'])
  expect(result.exitCode).toBe(2)
...
test('Group policy dry-run/apply is revision-safe and diagnostic evaluation creates no block', async () => {
  const exported = await server.cli(['group', 'policy', 'export', 'default'])
  expect(exported.exitCode).toBe(0)
  const document = JSON.parse(exported.stdout) as {
    expectedRevision: number
    edges: unknown[]
  }
  const path = file('group.json', document)
  let result = await server.cli(['group', 'policy', 'import', 'default', path, '--dry-run'])
  expect(result.exitCode).toBe(0)
  expect(result.stdout).toContain('valid: no changes applied')

  result = await server.cli(['group', 'policy', 'import', 'default', path, '--apply'])
  expect(result.exitCode).toBe(0)
  expect(result.stdout).toContain('updated Group default policy')

  result = await server.cli(['group', 'policy', 'import', 'default', path, '--apply'])
  expect(result.exitCode).toBe(3)
  expect(result.stderr).toContain('group_revision_conflict')
  ...
  const blocks = await server.cli(['group', 'policy', 'blocks', 'default', '--json'])
...
test('invalid interchange fails with a non-zero exit and no mutation', async () => {
  ...
  const result = await server.cli(['team', 'import', path, '--apply'])
  expect(result.exitCode).toBe(2)
  expect(result.stderr).toContain('unsupported harness document version')
  const list = await server.cli(['team', 'list', '--json'])
