import type { HarnessTemplateDetail } from '@bazilion/api-types'
import { describe, expect, test } from 'vitest'
import {
  exportTeamDocument,
  parseGroupPolicyDocument,
  parseTeamDocument,
  teamImportBody,
} from '../src/harness-interchange.ts'
...
test.each([
  ...
  [
    'invalid kind',
    { ...exportTeamDocument(detail), kind: 'bazilion.group-policy' },
...
test('Group policy validation rejects portable Agent self edges', () => {
  expect(() =>
    parseGroupPolicyDocument({
      version: 1,
      kind: 'bazilion.group-policy',
      groupId: 'g',
      expectedRevision: 1,
      edges: [
        {
