...
// Scheduled turns run in the protected container posture; without Docker the
// turn fails closed and the cycle never starts, so these tests are gated.
...
test.skipIf(!docker)(
  'CT-11: disable pauses cycles, re-enable resumes, delete stops them permanently',
  async () => {
    let rounds = 0
    mock.setFallback(
...
test.skipIf(!docker)(
  'CT-13 retry half: a failing turn retries with a bounded attempt count, then fails terminally',
  async () => {
    let providerHits = 0
    mock.setFallback(
      async (req: import('node:http').IncomingMessage, res: import('node:http').ServerResponse) => {
...
test.skipIf(!docker)(
  'CT-09 partial: a TZ=UTC daemon fires cron at the UTC minute, not the test-local minute',
  async () => {
    let cronRounds = 0
    mock.setFallback(
      async (req: import('node:http').IncomingMessage, res: import('node:http').ServerResponse) => {
