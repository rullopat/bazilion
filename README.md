...
bazilion team context <team> [--target <path>] [--json] # inspect repository instructions and command suggestions (unreleased)
...
- **Repository context (unreleased)** — inspect scoped repository `AGENTS.md`, Git state and command suggestions from the Team page or CLI. Coding turns receive daemon-prepared instructions through Pi; discovery does not enable extensions or execute project commands. See [repository context](docs/repository-context.md).
- **Agent-led coding (unreleased)** — ask an Agent to investigate, prepare available local prerequisites and run relevant checks during the task. Command results stay in chat; Team runtime defaults are optional. See [coding during an Agent task](docs/coding-environments.md).
